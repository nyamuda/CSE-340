...
function addFile()
{
    ...
    if ($imageCheck) {
        ...
        if ($result) {
            ...
            $message = '<p class="error-message">Sorry, the upload failed.</p>';
        }
    }
...
function removeFile()
{
    ...
    if ($remove) {
        ...
        $message = "<p class='error-message'>$filename was NOT deleted.</p>";
    }
...
function imagesList()
{
    ...
    if (count($imageArray)) {
        ...
        $imageDisplay = '<p class="error-message">Sorry, no images could be found.</p>';
    }
